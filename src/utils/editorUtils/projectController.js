...
import { createManifestFromState, validateManifest } from "./manifest";
...
async function processLoadedContent(content) {
  try {
...
Load anyway?`);
    ...
    projectStore.updateSettings({
      ...
      resolution: {
        ...
        label: `${manifest.project.resolution.width}x${manifest.project.resolution.height}`
        // Approximate label
      },
      fps: manifest.project.fps,
      seed: manifest.project.seed
    });
    const loadedClips = manifest.clips.map((mc) => ({
      id: mc.id,
      type: mc.type,
      // Cast to 'video'|'audio'|'image'
      path: mc.file,
      // Note: In Electron, this might need path resolution if relative
      filename: mc.file,
      // Usually just filename
      startFrame: mc.timelineIn,
      endFrame: mc.timelineOut,
      sourceDurationFrames: mc.metadata?.durationFrames || mc.sourceOut - mc.sourceIn,
      // Estimate if missing
