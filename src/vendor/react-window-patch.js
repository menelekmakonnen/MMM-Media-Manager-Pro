...
function $(e, t = "Assertion error") {
  if (!e) console.warn("Virtualization sanity check failed:", t);
}
function Y(e, t) {
  if (e === t)
    return !0;
  if (!!e != !!t || (e && t && Object.keys(e).length !== Object.keys(t).length))
    return !1;
  for (const s in e)
...
function me({
  ...
  return {
    get(n) {
      if (!e || e <= 0 || n < 0) return { size: 0, scrollOffset: 0 };
...
function Ee({
  ...
  const _cellComponent = e || (typeof s === 'function' || (s && s.$$typeof)) ? s : undefined;
  const y = de(t || {}), _cellProps = L(() => ({ ...y, data: itemData || y.itemData }), [y, itemData]), V = L(
    () => (_cellComponent ? le(_cellComponent, ue) : undefined),
    [_cellComponent]
  ), [h, z] = H(null), k = ve(h, l), {
    getCellBounds: S,
    getEstimatedSize: W,
...
function Ae({
  ...
  const _rowComponent = f || (typeof e === 'function' || (e && e.$$typeof)) ? e : undefined;
  const m = de(d || {}), _rowProps = L(() => ({ ...m, data: itemData || m.itemData }), [m, itemData]), b = L(
    () => (_rowComponent ? le(_rowComponent, ue) : undefined),
    [_rowComponent]
